...
/// </summary>
public class ValueDetailView2D
{
    private AvaPlot plot;
    private DetailViewConfig config;
    private List<(double[] x, double[] y)> dataArrays = new();
    private List<Scatter> scatters = new();
    private List<Crosshair> crosshairs = new();
    private Tooltip tooltip;

    public ValueDetailView2D(AvaPlot targetPlot, DetailViewConfig configuration)
    {
        plot = targetPlot;
    ...
    public void GenerateData(int pointCount)
    {
        ...
        for (int seriesIndex = 0; seriesIndex < config.DataSeries.Count; seriesIndex++)
        {
            ...
            {
                var xData = new double[pointCount];
                var yData = new double[pointCount];
                ...
                dataArrays.Add((xData, yData));
            }
            else
            {
                // Fallback for non-2D data - create uniform X coordinates
                var xData = new double[pointCount];
                var yData = new double[pointCount];
                ...
                {
                    xData[i] = i;
                    yData[i] = seriesConfig.DataGenerator(i, yData);
                }
    ...
    private void SetupPlot()
    {
        ...
        for (int i = 0; i < config.DataSeries.Count; i++)
        {
            ...
            var scatter = plot.Plot.Add.Scatter(new double[0], new double[0]);
    ...
    private void OnMouseMove(object? sender, PointerEventArgs e)
    {
        ...
        for (int seriesIndex = 0; seriesIndex < scatters.Count; seriesIndex++)
        {
            var scatter = scatters[seriesIndex];
            var seriesConfig = config.DataSeries[seriesIndex];
            
            if (scatter != null)
            {
                var nearest = scatter.GetNearestX(mouseLocation, plot.Plot.LastRender, (float)maxDistance);
